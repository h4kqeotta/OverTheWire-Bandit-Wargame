# 🐧 Bandit Level 13 → Level 14

## 🎯 Objetivo
> <img width="1104" height="323" alt="36" src="https://github.com/user-attachments/assets/76828eec-15f6-4895-8361-77fee2b7fe84" />


---

## 🔎 Enumeración


En este nivel no obtenemos directamente la contraseña del siguiente nivel. En su lugar, se nos proporciona una clave privada SSH.

🎯 Objetivo: utilizar la clave privada para conectarnos como bandit14 y acceder a la contraseña almacenada en /etc/bandit_pass/bandit14.

🧠 Concepto: autenticación mediante claves SSH y conexión a otros niveles de Bandit.


<img width="773" height="386" alt="37" src="https://github.com/user-attachments/assets/0c8756ac-75c4-4236-96a4-a9a7404befb6" />


<img width="742" height="409" alt="38" src="https://github.com/user-attachments/assets/eb405569-b303-4593-8a8f-e65e5a98ff18" />


<img width="621" height="377" alt="39" src="https://github.com/user-attachments/assets/4b10b7b5-38ed-4b8e-b5a7-d66e867e33f7" />


<img width="666" height="381" alt="40" src="https://github.com/user-attachments/assets/61dd1163-53d3-4be5-af26-7963a078f8ef" />

🚩 Flag

-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
